...
class MovingMnistLMDB(Dataset):
    ...
    def encode(self, x):
        if self.tokenizer is None:
            words = nltk.word_tokenize(x)  # Tokenize sentence into words
            m = np.int32(np.zeros((1, len(words))))

            for i in range(len(words)):
                if words[i] in self.vocab:
                    m[0, i] = self.vocab[words[i]]  # Use existing vocab
                else:
                    print(f"⚠️ Warning: '{words[i]}' not found in vocab. Using [UNK] token.")  # Debugging info
                    m[0, i] = self.vocab.get('[UNK]', self.vocab['[PAD]'])  # Handle missing words safely
...
class CATER(Dataset):
    def __init__(
        ...
        with open(os.path.join('input_avi', f'{split}_{mode}.json'), 'r') as fp:
        ...
        if dataset == 'caterv1':
            ...
            additional_words = ["moving", "left", "right", "jumping", "then", "are", "here", "there"]
            for word in additional_words:
                if word not in self.vocab:
                     self.vocab[word] = len(self.vocab)  # Assign new index
    ...
    def __getitem__(self, idx):
        ...
        video_path = os.path.join('input_avi', video_path)
...
#     @property
#     def pad_token_id(self):
#         return self.model._convert_token_to_id(self.model.pad_token)
#
#     @property
#     def sep_token_id(self):
#         return self.model._convert_token_to_id(self.model.sep_token)
#
#     def tokenize(self, caption):
#         token_ids = self.model.encode(caption, add_special_tokens=True)
#         return token_ids
...
#         )
#         self.db_txn = env.begin()
#         self._keys = [f"{i}".encode("ascii") for i in range(env.stat()["entries"])]
#
#         if percentage < 100.0:
#             retain_k: int = int(len(self._keys) * percentage / 100.0)
#             random.shuffle(self._keys)
...
#     ):
#         mode = 'ambiguous' if randomness else 'explicit'
#         with open(os.path.join('data/CATER-GEN-v2', f'{split}_{mode}.json'), 'r') as fp:
#             self.anno = json.load(fp)
#
#         self.data_root = data_root
#         self.transform = image_transform
...
#             'sliding': 7, 'picked': 8, 'placed': 9, 'containing': 10, 'rotating': 11, 'and': 12, 'to': 13,
#             'up': 14, 'sphere': 15, 'cylinder': 16, 'cube': 17, 'small': 18, 'medium': 19,
#             'large': 20, 'metal': 21, 'rubber': 22, 'gold': 23, 'gray': 24, 'red': 25,
#             'blue': 26, 'green': 27, 'brown': 28, 'purple': 29, 'cyan': 30, 'yellow': 31,
#             'rotating': 32, 'while': 33, 'contained': 34, 'still': 35,
#             'first': 36, 'second': 37, 'third': 38, 'fourth': 39, 'quadrant': 40
#         }
#
#         # Dynamically add missing words
#         additional_words = ["moving", "left", "right", "jumping", "then", "are", "here", "there"]
#         for word in additional_words:
#             if word not in self.vocab:
#                 self.vocab[word] = len(self.vocab)  # Assign new index
...
#         if self.tokenizer is None:
#             words = nltk.word_tokenize(x)
#             m = np.zeros((1, len(words)), dtype=np.int32)
#
#             for i in range(len(words)):
#                 if words[i] in self.vocab:
#                     m[0, i] = self.vocab[words[i]]
#                 else:
#                     print(f"⚠️ Warning: '{words[i]}' not found in vocab. Using [UNK] token.")
#                     m[0, i] = self.vocab.get('[UNK]', self.vocab['[PAD]'])
#
#             m = np.insert(m, 0, self.vocab['[CLS]'])
#             m = np.append(m, self.vocab['[SEP]'])
#         else:
#             m = self.tokenizer.tokenize(x)
#         return m
#
#     # def __getitem__(self, idx):
#     #     # 🔹 Get video path from annotations
#     #     video_path = os.path.normpath(os.path.join('data/CATER-GEN-v2', self.anno[str(idx)]['video']))
#     #     # video_path = os.path.normpath(os.path.join(self.data_root, "CATER-GEN-v1/videos", self.anno[str(idx)]['video']))
#     #     # video_path = 'data/CATER-GEN-v1/CATER-GEN-v1/videos'
#     #     caption = self.anno[str(idx)]['caption']
#     #
#     #     # 🔹 Debugging: Print video path
#     #     print(f"🔍 Checking video path: {video_path}")
#     #
#     #     # 🔹 Check if video exists
#     #     if not os.path.exists(video_path):
#     #         print(f"⚠️ WARNING: Video file missing: {video_path}")
...
#     #         vid = VideoReader(video_path)
#     #     except RuntimeError:
#     #         print(f"⚠️ WARNING: Cannot read video: {video_path}")
#     #         return None  # Skip corrupt videos
#     #
#     #     frame_num = len(vid)
#     #
...
#     #
#     #     # 🔹 Select frames
#     #     choice_idx = np.floor(np.linspace(0, frame_num - 1, round(frame_num / sample_interval), endpoint=True)).astype(
#     #         np.int32)
#     #     images = vid.get_batch(list(choice_idx)).asnumpy()
#     #     images = images[:self.frames_length]
#     #
#     #     # 🔹 Convert images to PIL and apply transforms
#     #     images = self.transform([Image.fromarray(i) for i in images]).permute(1, 0, 2, 3)
#     #
#     #     # 🔹 Pad if necessary
#     #     if images.shape[0] < self.frames_length:
#     #         images = torch.cat([images, images[-1].unsqueeze(0).repeat(self.frames_length - images.shape[0], 1, 1, 1)],
#     #                            dim=0)
#     #
#     #     # 🔹 Encode caption safely
#     #     caption_tokens = self.encode(caption)
#     #     caption_tokens = torch.tensor(caption_tokens, dtype=torch.long)
#     #
#     #     return {
...
#         images = images[:self.frames_length]
#
#         # 🔹 Ensure self.transform is valid before using
#         if self.transform is not None:
#             images = self.transform([Image.fromarray(i) for i in images]).permute(1, 0, 2, 3)
#         else:
#             print("⚠️ WARNING: `self.transform` is None. Using default normalization.")
#             images = torch.tensor(images, dtype=torch.float) / 255. - 0.5  # Normalize manually
#
#         if images.shape[0] < self.frames_length:
#             images = torch.cat([images, images[-1].unsqueeze(0).repeat(self.frames_length - images.shape[0], 1, 1, 1)],
#                                dim=0)
...
#
#         return {
#             'video_id': [d['video_id'] for d in data],
#             'images': torch.stack([d['images'] for d in data], dim=0),
#             'text': caption_tokens,
#             'speed': torch.stack([d['speed'] for d in data], dim=0)
#         }